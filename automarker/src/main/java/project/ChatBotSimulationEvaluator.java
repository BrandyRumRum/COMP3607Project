    ...
    private void evaluateMainMethod(Class<?> clazz) {
        try {
            ...
            if (mainMethod.toString().contains("addChatBot")) {
                ...
                feedback.add(
                        "The 'main' method should add several ChatBot objects (at least one of each kind) to the ChatBotPlatform.");
            }

            // Check for printing initial ChatBot list with summary statistics
            if (mainMethod.toString().contains("getChatBotList")
                    && mainMethod.toString().contains("System.out.print")) {
                totalScore += 2; // Award 2 marks for printing the ChatBot list
            } else {
                feedback.add(
                        "The 'main' method should print a list of all ChatBots managed by the ChatBotPlatform with their summary statistics.");
            }
            ...
            if (mainMethod.toString().contains("interactWithBot")) {
                ...
                feedback.add(
                        "The 'main' method should interact with ChatBots by sending messages and printing their responses to the screen.");
            }
            ...
            if (mainMethod.toString().contains("getChatBotList")
                ...
                feedback.add(
                        "The 'main' method should print the final list of all ChatBots managed by the ChatBotPlatform with their summary statistics.");
            }
